...
const DashboardScreen = (props) => {
  ...
  const getUserProfile = (bool) => {
    setLoading(bool);
    let tempArray = [];
    let url = `/api/v1/courses/?page=${page}&size=5&${isFreeKey}=${isFree}&categoryId=${categoryId}&title=%${title}%`;
    ApiHelper.getCoursesData(token, url, (response) => {
      if (response.isSuccess) {
        if (response.response.data.code === 200) {
          ...
          setCoursesData([...coursesData, ...tempArray]);
          setLoading(false);
        } else {
  ...
  const renderCourseItems = (item, index) => {
    return (
      <CourseCard
        ...
        price={item.price}
        onPressCourse={() =>
          props.navigation.navigate(COURSE_DETAILS, { courseId: item.id })
        }
      />
    );
  };
