...
const AllResourcesScreen = ({ navigation }) => {
  ...
  const getAllResources = (bool, doRefresh) => {
    setLoading(bool);
    let url = `/api/v1/resources/?size=15&page=${page}&${isFreeKey}=${isFree}&categoryId=${categoryId}&title=%${title}%`;
    ApiHelper.getResourceData(token, url, (response) => {
      if (response.isSuccess) {
        console.log("Data", response);
        if (response.response.data.code === 200) {
          ApiHelper.consoleBox(
          ...
          if (coursesData.length < 0) {
            ...
            setCoursesData([
              ...coursesData,
              ...response.response.data.data.docs,
            ]);
        ...
        try {
          const granted = await PermissionsAndroid.request(
            PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE,
            {
              ...
              message: "Acadify needs access to your storage to download File",
            }
          );
              ...
              return (
                <View style={styles.upperView}>
                  <TouchableOpacity
                    ...
                    onPress={() => setDropModal(!dropModal)}
                  >
          ...
          <View style={styles.upgradePlan}>
            <Text
              style={[
                ...
                { fontSize: wp(6), fontWeight: "500", textAlign: "center" },
              ]}
            >
              Upgrade Your Plan To Get Access
            </Text>
            <View style={{ marginTop: hp(2) }}>
              <Button
                ...
                width={wp(50)}
                onPress={() =>
                  navigation.navigate(PLAN_SCREEN, { fromChange: true })
                }
              />
            </View>
          </View>
